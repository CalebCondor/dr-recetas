...
export function useProductDetail(categorySlug: string, productSlug: string) {
  console.log("🎯 useProductDetail called with:", {
    categorySlug,
    productSlug,
  });
      ...
      try {
        ...
        for (const [categoryName, items] of Object.entries(allData)) {
          ...
          const match = items.find((item) => {
            ...
            try {
              if (
                decodeURIComponent(itemSlug) === decodeURIComponent(targetSlug)
              )
                return true;
            } catch {}
        ...
        if (foundProduct) {
          ...
          const allSlugs = Object.values(allData)
            .flat()
            .map((i: ApiServiceItem) => i.slug);
